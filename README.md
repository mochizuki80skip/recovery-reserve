...
## Threease への負荷（読みに行く回数）

- **空き状況**: 院ごと・7 日ブロックごとに、シフト 1 回＋予約 1〜2 ページ（＝2〜3 呼び出し）。60 秒以内は保存済みを返し、期限切れは前回分を即返して裏で 1 回だけ読み直す（同時アクセスはロックと進行中共有でまとめる）。誰も見ていない時間は読まない。院情報（スタッフ・コース・ユニット）は 1 時間に 3〜7 呼び出し
- **お客様データ**: 毎朝の cron で全件（三島 27 ページ＋裾野 64 ページ＝約 90 呼び出し／日）。管理画面は保存済みを返すだけで、「Threease から読み直す」を押した時だけ同じ量を読む
- **手動ブロック画面**: 空き状況と同じ保存済みブロックを使う（追加の読み込みなし）
- 実際の呼び出し数は管理画面「通信ログ」に日ごと（予約／お客様／シフト／その他の内訳、成功率、平均応答）で出る（`api/_threease.js` の `apiGet` で 1 呼び出しずつ記録）
...
- **新規継続率**: 対象期間（直近 3／6／12 ヶ月）に初回来院した新規のうち 2〜6 回目まで来た割合（初回担当別・最終担当別。初回から 60 日未満の人は除く。来院回数 = `reservation_count` − 次回予約 1 件）
- **離反率・継続率**: 離反率 = 最終来院が 120 日以内の人を最終担当別に、そのうち最終来院から 60〜120 日あいて次回予約が無い人の割合（＋離反全体に占める割合）。継続率 = 最終来院が 60 日未満の人を前回の担当別に、次回予約が入っている割合
- **症状別**（分析）: お客様情報の `customer_symptoms` ごとに 人数・平均来院回数・継続率（既存の次回予約あり）・離反率・新規の 2/3/6 回目到達率。複数症状の人は各症状に数える。未記入は「（未記入）」行。3 人以上の症状だけ表示（「すべて」で全部）
- **月次の数字**（分析）: 直近 12 か月の 新規（初回来院がその月）・離反（最終来院＋60 日がその月に入り、その後の来院も次回予約も無い）・純増・来院人数・来院回数・一人当たり来院回数・既存人数（当月＋前月に来院した人数）。来院系は `api/admin/monthly.js` が月ごとの予約（取消・仮予約を除く）を読んで Redis `recovery:monthly:{clinic}:{YYYY-MM}` に保存（終わった月は無期限、今月は 12 時間）。初回は 1 か月あたり 7〜12 秒かかる
- **手動ブロック**（設定）: 院・日付を選び、スタッフ×30 分の表で枠をタップして塞ぐ（「全員」列はその時間帯を全員分）。`api/busy.js`（GET で日の詳細＝出勤スタッフ・稼働区間・予約・ブロック、PUT で保存）、保存先 Redis `recovery:busy:{clinic}` = `{ 'YYYY-MM-DD': { '<staffId>'|'*': ['10:00', …] } }`（ローカルは `.local-busy-{clinic}.json`、前日より古い日付は保存時に削除）。空き計算 `computeDay` の `manual` でそのスタッフの 30 分を埋まり扱いにする
